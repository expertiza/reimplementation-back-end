class UpdateParticipantTypes < ActiveRecord::Migration[7.0]
  def self.up
    # add_column :participants, :type, :string

    begin
      execute "ALTER TABLE `participants` DROP FOREIGN KEY `fk_participant_assignments`"
    rescue StandardError
    end

    begin
      execute "ALTER TABLE `participants` DROP INDEX `fk_participant_assignments`"
    rescue StandardError
    end
