...
RSpec.describe "Api::V1::ReviewMappings", type: :request do
  ...
  # Tests for the check_outstanding_reviews endpoint
  path '/api/v1/review_mappings/check_outstanding_reviews' do
    get 'Checks if a reviewer has exceeded outstanding reviews limit' do
      tags 'Review Mappings'
      security [bearerAuth: []] # Requires JWT authentication
      produces 'application/json'
      ...
      response '200', 'review allowed' do
        ...
        before do
          # Create 1 review for the reviewer (below the limit of 2)
          create(:review_response_map,
                 reviewer_id: reviewer.id,
                 reviewed_object_id: assignment.id,
                 reviewee_id: team.id)
      ...
      # Test when reviewer has reached outstanding reviews limit
      response '200', 'review not allowed' do
        let(:assignment_id) { assignment.id }
        let(:reviewer_id) { reviewer.id }

        before do
          # Create 2 reviews for the reviewer (at the limit)
          2.times do
            create(:review_response_map,
                   reviewer_id: reviewer.id,
                   reviewed_object_id: assignment.id,
