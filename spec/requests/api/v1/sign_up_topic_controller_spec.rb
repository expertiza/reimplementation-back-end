...
RSpec.describe 'SignUpTopicController API', type: :request do
      ...
      response(200, 'successful') do
        ...
        context 'when assignment_id parameter is present' do
          ...
          context 'when topic_identifier parameter is present' do
            ...
            before do
              get "/api/v1/sign_up_topics?assignment_id=#{assignment_id}&topic_identifier=#{topic_identifier}"
            end
  ...
  path '/api/v1/sign_up_topics' do
    delete('Delete sign-up topics') do
      parameter name: 'assignment_id', in: :query, type: :integer, description: 'Assignment ID', required: true
      parameter name: 'topic_ids', in: :query, type: :array, items: { type: :string },
                description: 'Topic Identifiers to delete', required: false
      ...
      response(200, 'successful') do
        ...
        context 'when assignment_id parameter is missing' do
          ...
          before { delete '/api/v1/sign_up_topics', params: { assignment_id: } }
        ...
        context 'when assignment_id parameter is present' do
          context 'when topic_ids parameter is missing' do
            ...
            it 'deletes all sign-up topics with the given assignment_id' do
              ...
              expect(SignUpTopic.where(assignment_id:)).to be_empty
            end
          end
          ...
          context 'when topic_ids parameter is present' do
            ...
            before do
              delete "/api/v1/sign_up_topics?assignment_id=#{assignment_id}&topic_ids=#{topic_ids.join(',')}"
            end
            ...
            it 'deletes sign-up topics with the given assignment_id and topic_identifier' do
              ...
              expect(SignUpTopic.where(assignment_id:, topic_identifier: topic_ids)).to be_empty
            end
          end
        end
      end
  ...
  path '/api/v1/sign_up_topics' do
    ...
    context 'when the request is valid' do
      let(:valid_attributes) do
        { sign_up_topic: attributes_for(:sign_up_topic, assignment_id: assignment.id), micropayment: 0.1 }
      end
    ...
    context 'when the request is invalid' do
      let(:invalid_attributes) do
        { sign_up_topic: { topic_name: '' }, micropayment: 0.1, assignment_id: assignment.id }
      end
    ...
    context 'when the assignment does not exist' do
      let(:invalid_attributes) do
        { sign_up_topic: attributes_for(:sign_up_topic), micropayment: 0.1, assignment_id: 999 }
      end
