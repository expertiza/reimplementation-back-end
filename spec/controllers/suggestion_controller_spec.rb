describe SuggestionController do
  ...
  let(:assignment_form) { double('AssignmentForm', assignment:) }
  ...
  let(:assignment_questionnaire) { build(:assignment_questionnaire, id: 1, questionnaire:) }
  ...
  describe '#update_suggestion' do
    it 'checks updated is saved and redirect to the new' do
      ...
      request_params = { id: 1,
                         suggestion: { title: 'new title', description: 'new description', signup_preference: 'N' } }
