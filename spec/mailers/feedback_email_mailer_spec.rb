...
RSpec.describe FeedbackEmailMailer, type: :mailer do
  ...
  before do
    stub_const('Mailer', mailer_double)
    allow(mailer_double).to receive(:sync_message).and_return(mail_message)
  ...
  describe '#call' do
    ...
    it 'sends to the reviewer email address' do
      expect(mailer_double).to receive(:sync_message) do |defn|
