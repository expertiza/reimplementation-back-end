...
describe CourseNode do
  ...
  describe '#self.create_course_node(course)' do
	 it 'The code defines a method called "create_course_node" that takes a course object as an argument. It then creates a new CourseNode object, sets its "node_object_id" attribute as the courses id, gets a parent id for the course node, sets the parent id if it exists, and saves the course node to the database.' do
  ...
  describe '#self.table' do
	 it 'The ruby code  defines a class method called "table" that returns the string "courses".' do
   end
  end
  describe '#self.get' do
	 it 'The code defines a class method called "get" for a model, which takes several optional parameters. It checks if the "sortvar" parameter is a valid column name in the "Course" table. If it is, it queries the model with certain conditions and orders the results by the "sortvar" column in descending order.' do
   end
  end
  ...
  describe '#get_directory' do
	 it 'The Ruby code  defines a method `get_directory` which attempts to find a `Course` object by its `id` attribute, using the `find_by` method. If a matching `Course` object is found, the method returns the value of its `directory_path` attribute. If no matching `Course` object is found, the method returns `nil`.' do
   end
  end
  ...
  describe '#get_modified_date' do
	 it 'The code defines a method called `get_modified_date` which retrieves the `updated_at` attribute from a `Course` model object with an ID matching `node_object_id` (which is not defined in the code snippet) and returns it. If no such object is found, `nil` is returned.' do
   end
  end
  ...
  describe '#get_instructor_id' do
	 it 'The  Ruby code defines a method called `get_instructor_id`. This method finds a course with a specific `id` and returns the `instructor_id` associated with that course. If no course is found with the specified `id`, the method returns `nil`. The `try` method is used to avoid a `NoMethodError` if the `find_by` method returns `nil`.' do
   end
  end
