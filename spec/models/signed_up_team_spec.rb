...
RSpec.describe SignedUpTeam, type: :model do
    ...
    describe '.get_team_id' do
        ...
        it 'returns the correct team ID for the given user and assignment' do
          expect(SignedUpTeam.get_team_id(user.id, assignment.id)).to eq(team.id)
        end
        ...
        it 'returns nil if the user is not associated with any team for the assignment' do
          ...
          expect(SignedUpTeam.get_team_id(other_user.id, assignment.id)).to be_nil
        end
    end

    describe '.drop_off_team_signup_records' do
        ...
        it 'removes all sign-up records for the given team' do
          expect { SignedUpTeam.drop_off_team_signup_records(team.id) }
    ...
    describe '#reassign_topic' do
        ...
        before do
            allow(ProjectTopic).to receive(:find).with(project_topic.id).and_return(project_topic)
            allow(project_topic).to receive(:drop_team_from_topic)
        end

        it 'reassigns the team to a new topic and marks them as not waitlisted' do
            signed_up_team.reassign_topic(project_topic.id)
            expect(signed_up_team.reload.is_waitlisted).to be_falsey
            expect(project_topic).to have_received(:drop_team_from_topic).with(team_id: team.id)
        end
    end
end
