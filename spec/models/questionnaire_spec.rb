...
# Combined two tests into one to avoid performing the same functionalities again to test another feature.
  ...
  describe '#max_possible_score' do
    it 'returns 0 if there are no questions' do
      questionnaire = FactoryBot.create(:questionnaire, instructor: instructor)
      expect(questionnaire.max_possible_score).to eq(0)
    end

    it 'computes the max possible score for 1 question' do
      # Create a questionnaire with a single question
      questionnaire = FactoryBot.create(:questionnaire, instructor: instructor)
      question = FactoryBot.create(:question, questionnaire: questionnaire, weight: 0.5)

      expected_max_score = questionnaire.max_question_score * question.weight ## 10 * .5 = 5 so this should be 5.
      expect(questionnaire.max_possible_score).to eq(expected_max_score)
    end

    it 'computes the max possible score for 2 questions' do
      # Create a questionnaire with a single question
      questionnaire = FactoryBot.create(:questionnaire, instructor: instructor)
      question = FactoryBot.create(:question, questionnaire: questionnaire, weight: 0.5)
      question2 = FactoryBot.create(:question, questionnaire: questionnaire, weight: 0.6)

      expected_max_score = questionnaire.max_question_score * question.weight ## 10 * .5 + 10 * .6 = 11 so this should be 11.
      expect(questionnaire.max_possible_score).to eq(expected_max_score)
    end
  end
