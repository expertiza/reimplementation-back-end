...
module SummaryHelper
  class Summary
    ...
    def summarize_reviews_by_reviewee(questions, assignment, reviewee_id, summary_ws_url, _session = nil)
      ...
      questions.each_with_index do |question, index|
        ...
        question_iterator.each do |question| 
          next if question.question_type.eql?('SectionHeader')
    ...
    #   summary[round.to_s][question.txt] = summarize_sentences(break_up_comments_to_sentences(question_answers), summary_ws_url)
        summary[round.to_s][question.txt] = break_up_comments_to_sentences(question_answers)

    end

    def get_max_score_for_question(question)
      question.question_type.eql?('Checkbox') ? 1 : Questionnaire.where(id: question.questionnaire_id).first.max_question_score
    end
    ...
    def summarize_sentences(comments, summary_ws_url)
      ...
      begin
        ...
        sum_json = RestClient::Request.execute(
            method: :post,
            url: summary_ws_url,
            payload: param.to_json,
            headers: { content_type: :json, accept: :json },
            open_timeout: 5,  # seconds to wait for connection to open
