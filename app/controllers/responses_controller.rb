...
class ResponsesController < ApplicationController
  before_action :set_response, only: [:show, :update, :submit, :unsubmit, :destroy]
  ...
  def create
    ...
    # Reuse existing unsubmitted draft if one exists
    existing_draft = Response.where(map_id: @response_map.id, is_submitted: false).order(created_at: :desc).first
    if existing_draft
      return render json: {
        message: 'Existing draft found',
        response: existing_draft.as_json(include: { scores: { only: [:id, :item_id, :answer, :comments] } })
      }, status: :ok
    end
  ...
  def show
    ...
    render json: {
      response: @response.as_json(include: { scores: { only: [:id, :item_id, :answer, :comments] } })
    }, status: :ok
  end

  # GET /responses?map_id=123
  # Returns latest response (draft or submitted) for a given map_id
  def index
    map_id = params[:map_id]
    return render json: { error: 'map_id is required' }, status: :bad_request if map_id.blank?

    response = Response.where(map_id: map_id).order(created_at: :desc).first
    return render json: { response: nil }, status: :ok unless response
  ...
  def response_belongs_to?
    ...
    # reviewer is a Participant; compare its user_id to current_user.id
    return @response.map&.reviewer&.user_id == current_user.id if @response&.map&.reviewer && current_user
    ...
    map.reviewer&.user_id == current_user&.id
  end
