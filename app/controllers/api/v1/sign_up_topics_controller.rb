class Api::V1::SignUpTopicsController < ApplicationController
  ...
  def create
    ...
    #topic = ['a',1]
    topic = SignUpTopic.where(topic_name: params[:topic][:topic_name], assignment_id: params[:topic][:assignment_id]).first
    if topic.nil?
      ...
      @sign_up_topic.assignment_id = params[:topic][:assignment_id]
      @assignment = Assignment.find(params[:topic][:assignment_id])
