class Api::V1::SignUpTopicsController < ApplicationController
  ...
  def index
    if params[:assignment_id].nil?
      ExpertizaLogger.error LoggerMessage.new(controller_name, session[:user].name, "Assignment ID is missing.", request)
      ...
      ExpertizaLogger.info LoggerMessage.new(controller_name, session[:user].name, "Fetched sign-up topics for assignment ID: #{params[:assignment_id]}.", request)
      ...
      ExpertizaLogger.info LoggerMessage.new(controller_name, session[:user].name, "Fetched sign-up topics for assignment ID: #{params[:assignment_id]} with topic IDs: #{params[:topic_ids].join(', ')}.", request)
      render json: @sign_up_topics, status: :ok
    end
  ...
  def create
    ...
    if @sign_up_topic.save
      ...
      ExpertizaLogger.info LoggerMessage.new(controller_name, session[:user].name, "Created sign-up topic with ID: #{@sign_up_topic.id} for assignment ID: #{@assignment.id}.", request)
      render json: { message: "The topic: \"#{@sign_up_topic.topic_name}\" has been created successfully. " }, status: :created
    else
      ExpertizaLogger.error LoggerMessage.new(controller_name, session[:user].name, "Failed to create sign-up topic. Errors: #{@sign_up_topic.errors.full_messages.join(', ')}", request)
  ...
  def update
    if @sign_up_topic.update(sign_up_topic_params)
      ExpertizaLogger.info LoggerMessage.new(controller_name, session[:user].name, "Updated sign-up topic with ID: #{@sign_up_topic.id}.", request)
      render json: { message: "The topic: \"#{@sign_up_topic.topic_name}\" has been updated successfully. " }, status: 200
    else
      ExpertizaLogger.error LoggerMessage.new(controller_name, session[:user].name, "Failed to update sign-up topic with ID: #{@sign_up_topic.id}. Errors: #{@sign_up_topic.errors.full_messages.join(', ')}", request)
  ...
  def destroy
    ...
    if params[:assignment_id].nil?
      ExpertizaLogger.error LoggerMessage.new(controller_name, session[:user].name, "Assignment ID is missing for destroy action.", request)
      render json: { message: 'Assignment ID is required!' }, status: :unprocessable_entity
    elsif params[:topic_ids].nil?
      @sign_up_topics = SignUpTopic.where(assignment_id: params[:assignment_id])
      # render json: @sign_up_topics, status: :ok
    ...
    if @sign_up_topics.each(&:delete)
      ExpertizaLogger.info LoggerMessage.new(controller_name, session[:user].name, "Deleted sign-up topics.", request)
      render json: { message: "The topic has been deleted successfully. " }, status: :no_content
    else
      ExpertizaLogger.error LoggerMessage.new(controller_name, session[:user].name, "Failed to delete sign-up topics.", request)
      render json: @sign_up_topic.errors, status: :unprocessable_entity
    end
  end
