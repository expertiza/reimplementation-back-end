class Course < ApplicationRecord
  ...
  def path
    ...
    Rails.root + '/' + Institution.find(institution_id).name.gsub(' ',
  ...
  def add_ta(user)
    if user.nil?
      { success: false, message: "The user with id #{user.id} does not exist" }
    elsif TaMapping.exists?(ta_id: user.id, course_id: id)
      { success: false, message: "The user with id #{user.id} is already a TA for this course." }
    else
      ta_mapping = TaMapping.create(ta_id: user.id, course_id: id)
      user.update(role: Role::TEACHING_ASSISTANT)
      return { success: true, data: ta_mapping.slice(:course_id, :ta_id) } if ta_mapping.save

      { success: false, message: ta_mapping.errors }
  ...
  def remove_ta(ta_id)
    ta_mapping = ta_mappings.find_by(ta_id:, course_id: :id)
    ...
    ta_count = TaMapping.where(ta_id:).size - 1
    ta.update(role: Role::STUDENT) if ta_count.zero?
    ta_mapping.destroy
