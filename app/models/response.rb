...
class Response < ApplicationRecord
  ...
  def self.aggregate_assessment_scores(assessments, items)
    scores = {}
  ...
  def self.assessment_score(params)
    ...
    if @response
      ...
      weighted_score = 0
      sum_of_weights = 0
      @questionnaire = Questionnaire.find(items.first.questionnaire_id)

      questionnaire_data = ScoreView.questionnaire_data(items[0].questionnaire_id, @response.id)
  ...
  def self.review_grades(assignment, items)
    # puts items
    scores = { participants: {}, teams: {} }
    assignment.participants.each do |participant|
      scores[:participants][participant.id.to_s.to_sym] = scores_received_by_participant(participant, items)
      # puts participant.attributes
    end
    assignment.teams.each do |team|
      scores[:teams][team.id.to_s.to_sym] = scores_received_by_team(team, items)
  ...
  def self.scores_received_by_team(team, items)
    assignment = team.assignment
    scores = {}
    scores[:team] = {}
    # puts scores
    compute_assignment_score(team, items, scores, "ReviewResponseMap")
    scores
  end

 
  def self.scores_received_by_participant(participant, items)
    assignment = participant.assignment
    scores = {}
    scores[:participant] = {}
    # puts scores
    compute_assignment_score(participant, items, scores, "TeammateReviewResponseMap")
    # puts scores
  ...
  # retrieves the symbol of each questionnaire associated with a participant on a given assignment
  ...
  # Called in: scoring.rb
  def self.compute_assignment_score(reviewee, items, scores, type)
    reviewee.assignment.questionnaires.each do |questionnaire|
      round = AssignmentQuestionnaire.find_by(assignment_id: reviewee.assignment.id, questionnaire_id: questionnaire.id).used_in_round
      ...
      scores[questionnaire_symbol][:assessments] = if round.nil?
                                                     questionnaire.get_assessments_for(reviewee)
                                                   else
                                                     questionnaire.get_assessments_round_for(reviewee, round, type)
                                                   end
      # aggregate_assessment_scores computes the total score for a list of responses to a questionnaire
      # scores[questionnaire_symbol][:scores] = aggregate_assessment_scores(scores[questionnaire_symbol][:assessments], items[questionnaire_symbol])
    end
  end
...
def self.compute_grades_by_rounds(assignment, items, team)
  grades_by_rounds = {}
  total_score = 0
