class Duty < ApplicationRecord
  ...
  # check if the duty selected is available for selection in that particular team. 
  # Checks whether current duty count in the team is less than the max_members_for_duty 
  # set for that particular duty
  def can_be_assigned?(team)
    max_members_for_duty > team.participants.where(duty_id: id).count
  end
end
