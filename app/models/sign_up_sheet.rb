class SignUpSheet < ApplicationRecord
    # Team lazy initialization method 
    ...
    def self.add_signup_topic(assignment_id)
        # Retrieves and organizes due dates for all sign-up topics in a given assignment.
        @review_rounds = Assignment.find(assignment_id).num_review_rounds
        @topics = SignUpTopic.where(assignment_id: assignment_id)
        @duedates = {}
        
        # Return the empty hash if there are no topics
        return @duedates if @topics.nil?
      
        # Process each topic to retrieve and organize its due dates
        ...
        # Return the organized due dates
        @duedates
      end
      
  
    def self.has_teammate_ads?(topic_id)
      # Checks if there are any teams advertising for partners for a specific topic that are not yet full
    ...
    class << self
      ...
      def process_review_round(assignment_id, duedate, round, topic)
        # Processes and sets the submission and review due dates for a specific review round and topic
        ...
        # Format and set the submission and review due dates in the duedate hash
      ...
      def find_topic_duedates(round, topic)
        # Finds and returns the submission and review due dates for a specific review round and topic.
