...
module Reports
  ...
  #   state_key_for  — [map_id, round]: deduplication key (one response per map per round)
  ...
  class FeedbackReport < BaseReport
    def source
      Response
        .joins(:response_map)
        .where(response_maps: { type: 'ReviewResponseMap', reviewed_object_id: @reportable.id })
        .order(created_at: :desc)
    end

    def state_key_for = ->(r) { [r.map_id, r.round] }
    ...
    def accumulate(state, key, response)
      ...
      if @reportable.varying_rubrics_by_round?
        case response.round
    ...
    def fetch_authors
      teams = AssignmentTeam.includes(:users).where(parent_id: @reportable.id)
      teams.flat_map do |team|
        team.users.filter_map do |user|
          AssignmentParticipant.find_by(parent_id: @reportable.id, user_id: user.id)
        end
      end
    end
