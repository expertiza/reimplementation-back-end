class SignedUpTeam < ApplicationRecord
  ...
  def self.find_team_participants(assignment_id, ip_address = nil)
    # Retrieves participants for a specific assignment, including their team and user details.
    # Joins SignedUpTeam with sign_up_topics to get topic details.
    # For each participant, fetches team & user names and formats them.
    # Updates participants with team and user name information and returns the list.
  ...
  def self.find_team_users(assignment_id, user_id)
    # Finds the team IDs of the teams in a given assignment that a specific user is part of
  ...
  def self.find_user_signup_topics(assignment_id, team_id)
    # Retrieves the topics that a specific team has signed up for in a given assignment.
  ...
  def self.topic_id_by_team_id(team_id)
    # Retrieves the topic ID that a specific team (not waitlisted) has signed up for
