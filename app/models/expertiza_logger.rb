...
class ExpertizaLogger
  LOG_LEVELS = %i[info warn error fatal debug].freeze

  class << self
    LOG_LEVELS.each do |level|
      define_method(level) do |message = nil|
        logger(level).public_send(level, message)
      end
    end
    ...
    def logger(level)
      instance_variable_get("@#{level}_log") || instance_variable_set("@#{level}_log", create_logger(level))
    end

    def create_logger(level)
      logger = Logger.new(Rails.root.join('log', "expertiza_#{level}.log"))
      logger.formatter ||= ExpertizaLogFormatter.new
      logger
    end
  end
end
