class SignUpTopic < ApplicationRecord
  ...
  def self.find_slots_filled(assignment_id)
    # retrieves the number of filled slots for each topic in a specific assignment, excluding the waitlisted team
  ...
  def self.find_slots_waitlisted(assignment_id)
    # retrieves the number of waitlisted slots for each topic in a specific assignment
  ...
  def self.find_waitlisted_topics_for_team(assignment_id, team_id)
    # retrieves the IDs of the topics that a specific team is waitlisted for in a specific assignment
  ...
  def self.assign_to_first_waiting_team(next_wait_listed_team)
    # get the team_id from the next waitlisted team
    ...
    # cancels all other waitlist entries for this team in the same assignment
  ...
  def self.has_suggested_topic?(assignment_id)
    ...
    # returns true if there are suggested topics (non-private topics), false otherwise
    sign_up_topics.size != all_topics.size
  end
  

  def users_on_waiting_list
    # Finds all teams signed up for the current topic (identified by 'id') that are waitlisted
    waitlisted_signed_up_teams = SignedUpTeam.where(topic_id: id, is_waitlisted: 1)
    # Initializes an empty array to store the waitlisted users
    waitlisted_users = []
    # Checks if there are any waitlisted teams
    if waitlisted_signed_up_teams.present?
      # Iterates over each waitlisted team
      waitlisted_signed_up_teams.each do |waitlisted_signed_up_team|
        # Finds the assignment team using the team_id from the waitlisted signed-up team
        assignment_team = AssignmentTeam.find(waitlisted_signed_up_team.team_id)
        # Adds the users of the assignment team to the waitlisted_users array
        waitlisted_users << assignment_team.users
      end
    end
    # Flattens the array of arrays into a single array and returns it
